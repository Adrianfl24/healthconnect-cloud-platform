...
• Database Connectivity & Networking: Configured Azure SQL Server firewall rules
to allow traffic only from internal Azure services (0.0.0.0 rule). I also updated the
JDBC connection string parameters (encrypt=true, trustServerCertificate=false) to
fix HikariCP handshake timeouts inside the cluster.
...
# Open SQL firewall for internal Azure traffic
...
2. Registry Login: Authenticates to Azure ACR using GitHub Secrets (ACR_USERNAME,
ACR_PASSWORD).
3. Docker Build & Push: Builds the image, tags it with the Git Commit SHA, and pushes
it to ACR.
4. Deploy: Connects to AKS using the KUBECONFIG secret, applies the manifests, and exe-
cutes kubectl rollout restart deployment to fetch the new image.
Future Roadmap
• Observability: Add Prometheus and Grafana for metrics visualization.
• Security: Migrate database credentials from Kubernetes Secrets to Azure Key Vault (via
CSI driver).
• Ingress: Replace the basic LoadBalancer with an NGINX Ingress Controller and configure
